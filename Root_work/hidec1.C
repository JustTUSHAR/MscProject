//Higgs decay to neutrino and pair of muons(For emission of excited state and neutrino first)
...
{	
		n.SetE(i); //Energy of neutrino
		u.SetE((125020-i)/2.0); //Taking energy of muon half of what is left \\125.02GeV=125020MeV
		au.SetE(u.E()); //Energy of antimuon rest half or equal to muon
		...
		//u.SetPy(sqrt(pow(u.E(),2.0)-pow(u.Px(),2.0)-pow(105.6,2.0)));//Calculating y momentum using energy momemtum relation for muon
		u.SetPy(sqrt(pu-pow(u.Px(),2.0)));
...
gh->SetTitle("Muon pair angle distribution(For excited particle case);Energy of neutrino(In Mev);Angle between muon pair(Angle in radians)"); //Plotting the data
gh->Draw("A*");
}
